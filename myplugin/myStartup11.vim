...
function! <sid>onVimEnter()
    ...
    autocmd BufWinEnter *  :call <sid>onBufWinEnter()
    augroup END
endfunction

function! <sid>onBufEnter()
    "call mywindow#echoMoreMsg('...BufEnter:' .bufname('%'))
    call <sid>updateOldfiles()
endfunction!
function! <sid>onBufRead()
    "call mywindow#echoMoreMsg('...BufRead:' .bufname('%'))
    call <sid>updateOldfiles()
endfunction!
function! <sid>onBufWinEnter()
    "call mywindow#echoMoreMsg('...BufWinEnter:' .bufname('%'))
    call <sid>updateOldfiles()
endfunction!

function! <sid>updateOldfiles()
    "call mywindow#echoMoreMsg('bufname: ' .bufname('%') )
    "call mywindow#echoWarningMsg('...updateOldfiles <afile>: ' .expand('<afile>')  )
    if bufname('%')=='__ls__'       |return |endif
    if bufname('%')=='__old__'      |return |endif
    if bufname('%')=='__shortcut__' |return |endif
    if bufname('%')=='__session__'  |return |endif
    if bufname('%')=='__startup__'  |return |endif
    ...
    "call mywindow#echoMoreMsg('map...')
    call map(v:oldfiles, 'fnamemodify(v:val, ":p:~")')
    "echo v:oldfiles
    "call mywindow#echoMoreMsg('add ' .file .' to oldfiles')
endfunction
