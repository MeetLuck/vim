...
" " for syntax edit  ~/.vim/syntax/mysession.vim"
"~/.vim/autoload/mywindow.vim
"~/.vim/autoload/mylib.vim
...
function! <sid>closeWindow()
    let bufname = expand('%')
    call mywindow#echoMoreMsg('...moving to previous winnr '.s:previouswinnr)
    call mywindow#echoMoreMsg('...moving to previous winnr '.winnr('#'))
    autocmd! * <buffer>
    exe winnr('#') ."wincmd w" | exe bufwinnr(bufname) ."wincmd q"
...
function! <sid>createWindow(bufname)
    ...
    "autocmd! * <buffer>
    "autocmd CmdlineEnter <buffer> :call <sid>onCmdlineEnter()
    autocmd CmdlineEnter <buffer> :call <sid>closeWindow()
    autocmd BufLeave <buffer> :call mywindow#echoMoreMsg('...BufLeve') | wincmd q
endfunction

function! <sid>onCmdlineEnter()
    let bufname = expand('%')
    call mywindow#echoMoreMsg('...moving to previous winnr '.s:previouswinnr)
    call mywindow#echoMoreMsg('...moving to previous winnr '.winnr('#'))
    autocmd! * <buffer>
