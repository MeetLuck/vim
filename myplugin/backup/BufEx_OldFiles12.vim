...
if has('win32') 
    command! -buffer Reinstall :exe "!del " . expand("~/.vim/plugin/") . "BufEx_OldFiles*.vim"
...
function! <sid>OpenwindowPre(bufname)
    ...
    elseif s:prevwinnr != bufwinnr(a:bufname)
        let s:prevwinnr = winnr()
    elseif s:prevwinnr == bufwinnr(a:bufname)
        :EchoWarningmsg previous window is __shortcut__ window.
    else
        :EchoWarningmsg Unexpected Error in <sid>OpenWinPre()
    endif
endfunction

function! <sid>Closewindow(bufname)
    autocmd! * <buffer>
    echohl Warningmsg |echomsg 'to previous window' |echohl None | exe s:prevwinnr ."wincmd w"
