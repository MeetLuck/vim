...
function! <sid>createWindow(bufname)
    ...
    autocmd BufLeave <buffer> call mywindow#echoWarningMsg('...BufLeve') | call <sid>onBufWinLeave()
    autocmd WinLeave <buffer> call mywindow#echoWarningMsg('...WinLeve') | call <sid>onBufWinLeave()
endfunction

function! <sid>onBufWinLeave()
    autocmd! * <buffer>
    let curbufname = bufname('%')
    call mywindow#echoMoreMsg('bufname,bufwinnr,previouswinnr :' .s:bufname .' ' .s:bufwinnr .' ' .s:previouswinnr)
    call mywindow#echoMoreMsg('curbufname,curbufwinnr,winnr(#) :' .bufname('%') .' ' .winnr() .' ' .winnr('#'))
    if !mywindow#isCurrentWindow(s:bufwinnr)
        call mywindow#echoWarningMsg('current windowNr is NOT '.s:bufwinnr)
    elseif !mywindow#isCurrentBuffer(s:bufname)
        call mywindow#echoWarningMsg('current bufname is NOT '.s:bufname)
    else
        call mywindow#echoWarningMsg('current winnr,current bufname -> '.winnr() .', '.bufname('%'))
