...
function! <sid>Open(bufname)
    ...
    "Bufferhidden:
    throw 'because bufhidden is nofile,'
        \.'when the buffer is no longer displayed in a window'
        \.'the buffer cannot exist'
    echoerr 'please check out mapping for e.g. nmap <Leader>ls <nop>'
endfunction
...
function! <sid>closeWindow()
    ...
    if win_getid(winnr()) == s:winids.created
        exe win_id2win(s:winids.home) ."wincmd w" | exe win_id2win(s:winids.created) ."wincmd q"
    else
        throw 'current windows is wrong'
