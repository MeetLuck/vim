...
function! <sid>onVimEnter()
    ...
    autocmd BufEnter,BufRead,BufWinEnter  *  :call <sid>updateOldfiles()
endfunction
...
function! <sid>updateOldfiles()
    ...
    call filter(v:oldfiles,'v:val !~ "__ls__"')
    call filter(v:oldfiles,'v:val !~ "__old__"')
    call filter(v:oldfiles,'v:val !~ "__session__"')
    call filter(v:oldfiles,'v:val !~ "__startup__"')
    "continue...
